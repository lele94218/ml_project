#!/usr/bin/env python
from scipy.cluster import hierarchy
import numpy as np
import matplotlib.pyplot as plt
import hdbscan
from sklearn.datasets import make_blobs
import seaborn as sns
from sklearn.cluster import KMeans

# data = np.loadtxt("data/wine.data.txt", delimiter=',')
# cluster = np.array(data[:,0], dtype=int)
# data_set = data[:,1:]

"""
            wine        iris        yeast
HDBSCAN     49.3%       89.3%       31.1%
KMEANS      44.9%       66.6%       20.3%
"""


# data = np.loadtxt("data/iris.data.txt", delimiter=',')
# cluster = np.array(data[:,-1])
# data_set = data[:,:-1]

data = np.loadtxt("data/yeast.data.txt")
cluster = np.array(data[:,-1])
data_set = data[:,:-1]


kmeans = KMeans(n_clusters=3, random_state=0).fit(data_set)
print(kmeans.labels_)
#
# cluster[cluster==0] = 5
# cluster[cluster==1] = 0
# cluster[cluster==5] = 1
#
# print(cluster)
#
cnt = 0
for i in range(0, cluster.size):

    if cluster[i] == kmeans.labels_[i] - 1:
        # print("%s %s" %(cluster[i], clusterer.labels_[i]))
        cnt = cnt + 1

print (cnt / cluster.size)

# data_set, _ = make_blobs(1000)
# plt.scatter(data_set.T[0], data_set.T[1], color='b')
# plt.show()

clusterer = hdbscan.HDBSCAN(min_cluster_size=9)
cluster_labels = clusterer.fit_predict(data_set)
print(cluster)
print(cluster_labels)


# clusterer = hdbscan.HDBSCAN(algorithm='best', alpha=1.414, approx_min_span_tree=True,
#     gen_min_span_tree=False, leaf_size=40, metric='euclidean', min_cluster_size=5, min_samples=None, p=None)
# clusterer.fit(data_set)

print(cluster_labels[cluster_labels==2].size)
print(cluster_labels.size)
cnt = 0
for i in range(0, cluster.size):

    if cluster[i] == cluster_labels[i]:
        # print("%s %s" %(cluster[i], clusterer.labels_[i]))
        cnt = cnt + 1

print (cnt / cluster.size)
# clusterer.condensed_tree_.plot(select_clusters=True, selection_palette=sns.color_palette())
# plt.show()